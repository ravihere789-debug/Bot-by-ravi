...
SPEED_PRESETS = {
    "slow":   {"workers": 2,  "min_delay": 8.0,  "max_delay": 15.0, "action_pause": 3.0},
    "normal": {"workers": 5,  "min_delay": 3.0,  "max_delay": 6.0,  "action_pause": 1.5},
    "fast":   {"workers": 10, "min_delay": 1.5,  "max_delay": 3.0,  "action_pause": 0.8},
    "ultra":  {"workers": 15, "min_delay": 0.8,  "max_delay": 2.0,  "action_pause": 0.4},
    # Wide random jitter makes timing look human — conservative and safe
    "smart":  {"workers": 15, "min_delay": 0.3,  "max_delay": 2.5,  "action_pause": 0.6},
}
    ...
    async def _do_action() -> dict:
        try:
            ...
            if action == "bot_referral":
                import os as _os, re as _re, httpx as _httpx, urllib.parse as _uparse
                ...
                bot_un   = None
                st_param = None
                m = _re.match(r"https?://t\.me/([A-Za-z0-9_]+)(?:/\w+)?\?start(?:app)?=([^\s&]+)", link)
                if m:
                    bot_un, st_param = m.group(1), m.group(2)
                else:
                    m = _re.match(r"https?://t\.me/([A-Za-z0-9_]+)", link)
                    if m:
                        bot_un = m.group(1)
                ...
                # ── Helper: extract channels + callback buttons from messages ─
                def _extract(msgs):
                    chans = set()
                    cbs   = []
                    seen_cb = set()
                    _tme = _re.compile(
                        r"https?://(?:t|telegram)\.me/([A-Za-z0-9_+][A-Za-z0-9_+/]*)"
                    )
                    _at  = _re.compile(r"@([A-Za-z0-9_]{4,})")
                    for msg in msgs:
                        # Scan both text AND caption (photo/video messages use caption)
                        for body in (msg.text, msg.caption):
                            if not body:
                                continue
                            for ch in _tme.findall(body):
                                if ch.lower() != bot_un.lower():
                                    chans.add(
                                        f"https://t.me/{ch}" if ch.startswith("+") else f"@{ch}"
                                    )
                            for un in _at.findall(body):
                                if un.lower() != bot_un.lower():
                                    chans.add(f"@{un}")
                        if msg.reply_markup and hasattr(msg.reply_markup, "inline_keyboard"):
                            for row in msg.reply_markup.inline_keyboard:
                                for btn in row:
                                    if btn.url:
                                        m2 = _tme.search(btn.url)
                                        if m2:
                                            ch = m2.group(1)
                                            if ch.lower() != bot_un.lower():
                                                chans.add(
                                    ...
                                    elif btn.callback_data and btn.callback_data not in seen_cb:
                                        cbs.append((msg.id, btn.callback_data, btn.text or ""))
                                        seen_cb.add(btn.callback_data)
                    return chans, cbs

                # ── STEP 1: Snapshot existing history before /start ──────────
                # If the bot has been started before it won't reply again, so
                # we save the pre-existing messages as a fallback.
                pre_msgs = []
                async for msg in client.get_chat_history(bot_un, limit=20):
                    if msg.from_user and not msg.outgoing:
                        pre_msgs.append(msg)
                    if len(pre_msgs) >= 10:
                        break
                pre_ids = {m.id for m in pre_msgs}

                # ── STEP 2: Send /start via messages.StartBot (deep-link) ───
                # send_message("/start param") is plain text — bots ignore it
                # for referral counting.  messages.StartBot is the real MTProto
                # method that fires when a user clicks a t.me/bot?start=PARAM link.
                try:
                    ...
                    # Fallback: plain-text send (works for bots that don't
                    # distinguish deep-link vs text command).
                    start_text = f"/start {st_param}" if st_param else "/start"
                    await client.send_message(bot_un, start_text)
                await asyncio.sleep(0.8)

                # ── STEP 3: Read bot's reply (new messages only) ─────────────
                # If bot didn't reply (already started), fall back to pre-existing msgs.
                new_after_start = []
                async for msg in client.get_chat_history(bot_un, limit=10):
                    if msg.from_user and not msg.outgoing and msg.id not in pre_ids:
                        new_after_start.append(msg)
                    if len(new_after_start) >= 5:
                        break

                if new_after_start:
                    # Bot sent a fresh reply — use it
                    bot_msgs = new_after_start
                else:
                    # Bot was already started and didn't reply again — use history
                    bot_msgs = pre_msgs[:3]

                seen_msg_ids = pre_ids | {m.id for m in new_after_start}

                channels_found, cb_buttons = _extract(bot_msgs)

                # ── STEP 3: Join channels FIRST (bot checks real membership) ─
                joined = 0
                ...
                # Brief pause so bot's membership check can propagate
                if channels_found:
                    await asyncio.sleep(0.3)

                # ── STEP 4: Click all verify/join buttons immediately ─────────
                # No AI call — just click everything found; bots ignore
                # buttons that don't apply, and this is what the AI fell
                # back to anyway.
                verify_rounds = 0
                for msg_id, cb_data, _ in cb_buttons[:5]:
                    try:
                        await client.request_callback_answer(
                            chat_id=bot_un, message_id=msg_id, callback_data=cb_data,
                        )
                        verify_rounds += 1
                        await asyncio.sleep(action_pause)
                    except Exception:
                ...
                # ── STEP 5: One post-join verification round ──────────────────
                # Many bots send a "✅ Check membership" message after you join.
                await asyncio.sleep(1.0)   # give bot time to react

                new_msgs = []
                async for msg in client.get_chat_history(bot_un, limit=10):
                    if msg.from_user and not msg.outgoing and msg.id not in seen_msg_ids:
                        new_msgs.append(msg)
                    if len(new_msgs) >= 5:
                        break

                if new_msgs:
                    seen_msg_ids.update(m.id for m in new_msgs)
                    new_chans, new_cbs = _extract(new_msgs)

                    # Join any newly mentioned channels
                    for ch in new_chans:
                    ...
                    for msg_id, cb_data, _ in new_cbs[:5]:
                        try:
                            await client.request_callback_answer(
                                chat_id=bot_un, message_id=msg_id, callback_data=cb_data,
                            )
                ...
                return {
                    "ok": True,
                    "detail": (
                        f"Bot started · {joined} channel(s) joined"
                        f" · {verify_rounds} verify step(s) completed"
                    ),
                }
