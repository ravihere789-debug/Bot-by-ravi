...
SPEED_PRESETS = {
    ...
    'smart':  {'workers': 7, 'min_delay': 1.5,  'max_delay': 4.0},
}
...
def _reaction_emoji(value) -> Optional[str]:
    """Return a standard emoji from a Pyrogram reaction-like object."""
    if isinstance(value, str):
        return value
    return getattr(value, "emoji", None) or getattr(value, "emoticon", None)


async def _premium_reaction_choices(client, peer, message=None) -> list[str]:
    """Resolve the first two standard reactions enabled for a post's chat.

    Telegram exposes chat-specific reaction settings through ``Chat``.  Some
    accounts/older Pyrogram builds only hydrate the reactions already present
    on the message, so that is used as a secondary source before falling back
    to Telegram's standard reaction order.
    """
    choices: list[str] = []
    ...
    try:
        ...
        for reaction in getattr(available, "reactions", None) or []:
            emoji = _reaction_emoji(reaction)
            if emoji and emoji not in choices:
    ...
    if not choices:
        choices.extend(_PREMIUM_REACTION_FALLBACKS)
        ...
        async with client:
            if action == 'dm':
                ...
                if isinstance(post_channel, int) and joined_chat is not None:
                    peer = _leave_chat_peer(joined_chat) or peer
        ...
        async with semaphore:
            ...
            if res['ok']:
                ...
                if res['fatal']:
                    status = (
                        'reauth_required'
                        if _needs_reauthentication(res.get('error'))
                        else 'dead'
                    )
                    storage.set_account_status(user_id, idx, status)
