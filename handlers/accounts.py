...
async def check_account_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    _FATAL = {
        ...
        "FROZEN_METHOD_INVALID", "ACCOUNT_BANNED", "USER_BANNED",
    }

    def _is_fatal(exc: Exception) -> bool:
        upper = str(exc).upper().replace(" ", "_")
        return any(k in upper for k in _FATAL)

    async def _verify(session_str: str) -> str:
        """Returns 'active', 'frozen', 'banned', or 'dead'."""
        ...
        try:
            ...
            async with Client(
                ...
                if getattr(me, "is_deleted", False):
                    return "dead"
                # Write probe — frozen/banned accounts fail here
                try:
                    msg = await c.send_message("me", "\u200b")
                    try: await c.delete_messages("me", msg.id)
                    except Exception: pass
                    return "active"
                except Exception as probe_exc:
                    upper = str(probe_exc).upper().replace(" ", "_")
                    if "FROZEN" in upper:
                        return "frozen"
                    if "BANNED" in upper or "DEACTIVATED" in upper:
                        return "banned"
                    if any(k in upper for k in _FATAL):
                        return "dead"
                    return "active"  # transient error — assume still active
        except EOFError:
            return "dead"
        except Exception as e:
            if _is_fatal(e):
                upper = str(e).upper().replace(" ", "_")
                if "FROZEN" in upper:
                    return "frozen"
                if "BANNED" in upper or "DEACTIVATED" in upper:
                    return "banned"
                return "dead"
            return "active"  # transient / network — keep as active
