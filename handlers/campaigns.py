...
from keyboards import (
    ...
    campaign_action_types_menu, reaction_picker_menu, REACTIONS,
...
REACT_PICKER_ACTIONS = {"react", "react_vote", "react_view"}
...
async def campaign_view(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    react_line = f"⭐ <b>Reactions:</b> {' '.join(_reactions)}\n" if _reactions and camp.get("action_type","") in REACT_ACTIONS else ""
...
def _save_campaign(user_id: int, action: str, target: str, reactions: list = None, message: str = "", join_link: str = "", button_index: int = 0, max_accounts: int = 0) -> dict:
    import uuid
    from datetime import datetime
...
async def camp_get_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action in REACT_PICKER_ACTIONS:
        await query.edit_message_text(
            f"🚀 Campaign — {label}\n"
            "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n\n"
    ...
    # react_vote_view: skip the reaction picker — use 👍 as default and go
    # straight to the post URL step (matching the 2-step UX in the screenshot).
...
async def _do_save_camp(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for key in ["camp_action", "camp_target", "camp_reactions", "camp_dm_message",
...
async def campaign_run(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async def on_progress(done: int, failed: int, skipped: int, tot: int, errors: list) -> None:
        now = time.monotonic()
        finished = done + failed
    ...
    async def _bg() -> None:
        try:
            result = await execute_campaign(
                ...
                max_accounts_override=0,
                resume_identifiers=resume_ids,
            )
        finally:
