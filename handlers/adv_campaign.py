...
from keyboards import back_button, cancel_button, campaign_action_types_menu, reaction_picker_menu, REACTIONS, vote_button_picker_menu
...
async def _run_adv(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    from runner import run_campaign_on_account, progress_bar, SPEED_PRESETS
    from storage import (
    ...
    preset    = SPEED_PRESETS.get(speed_key, SPEED_PRESETS["smart"])
    ...
    counters = {"done": 0, "failed": 0, "errors": [], "stopped": False, "paused": False}
    lock      = asyncio.Lock()
    ...
    async def run_one(acc: dict, idx: int):
        async with semaphore:
            ...
            result = await run_campaign_on_account(
                identifier, action, target, dm_msg,
                reactions=[_assigned[idx]],
                join_link=join_link,
